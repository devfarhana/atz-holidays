...
if ($('#plan-overview').length > 0) {
  ...
  if ($('#transcation_chart').length > 0) {
    ...
    var chart = new ApexCharts(
      document.querySelector("#transcation_chart"),
      sColStacked
    );
   ...
   if ($('#tours_chart').length > 0) {
    ...
    let normalizedSeries = originalSeries.map(value => (value / maxValue) *100);
    ...
    radialBar: {
      ...
      barLabels: {
        ...
        formatter: function(seriesName, opts) {
          return seriesName + ":  " + originalSeries[opts.seriesIndex]
        },
      },
    }
  },
  ...
  var chart = new ApexCharts(document.querySelector("#tours_chart"), options);
  chart.render();
  chart.updateOptions({
    plotOptions: {
        radialBar: {
            max: 10 // Set max to 10 to fully fill the segment
        }
    }
});
}
